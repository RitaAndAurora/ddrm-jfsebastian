...
When navigating the different presets of a bank you'll see how the sliders on the screen move accordingly to represent the parameter values corresponding to the selected preset. You'll also see how the selected point in the Timbre Space moves at each preset change. This indicates the point in the Timbre Space that corresponds to the selected preset (more info in [Section 2.4](TODO:link)).
...
When a button is selected it will be highlighted. However, if any parameter is changed after the button is selected, it will become *unselected* again to indicate that the slider values are no longer *in sync* with the expected values for channel I or channel II selected button.

Note that `Funky 4` in the lower row does not work. We don't know what happened but the 
